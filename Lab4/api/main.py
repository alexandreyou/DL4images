...
def get_dataloaders(image_size: int, batch_size: int):
    ...
    train_dataset = datasets.ImageFolder(
        TRAIN_DIR, transform=get_transforms(image_size, augment=True)
    )
    val_dataset = datasets.ImageFolder(
        VAL_DIR, transform=get_transforms(image_size, augment=False)
    )
...
def train(req: TrainRequest):
    ...
    model = MODEL_REGISTRY[req.model_name](
        in_channels=3, dropout_rate=req.dropout_rate
    ).to(DEVICE)

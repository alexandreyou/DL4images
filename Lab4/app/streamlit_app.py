...
with st.sidebar:
    ...
    selected_model = st.selectbox(
        "Architecture", MODEL_LIST, label_visibility="collapsed"
    )
    ...
    dropout_rate = st.slider(
        "Dropout", min_value=0.0, max_value=0.8, value=0.5, step=0.1
    )
...
def plot_curves(results: dict, model_name: str) -> plt.Figure:
    ...
    # Loss
    axes[0].plot(
        epochs_range, results["train_losses"], label="Train", marker="o", markersize=3
    )
    axes[0].plot(
        epochs_range, results["val_losses"], label="Val", marker="o", markersize=3
    )
    ...
    # Accuracy
    axes[1].plot(
        epochs_range, results["train_accs"], label="Train", marker="o", markersize=3
    )
    axes[1].plot(
        epochs_range, results["val_accs"], label="Val", marker="o", markersize=3
    )
    ...
    # AUC
    axes[2].plot(
        epochs_range, results["train_aucs"], label="Train", marker="o", markersize=3
    )
    axes[2].plot(
        epochs_range, results["val_aucs"], label="Val", marker="o", markersize=3
    )
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    if train_btn:
        ...
        with st.spinner(
            f"Training {selected_model} for {epochs} epochs… this may take a while."
        ):
            try:
                ...
                st.error(
                    "Cannot reach the API. Is `uvicorn main:app --reload --port 8000` running?"
                )
    ...
    if (
        ...
        and st.session_state.last_model_name == selected_model
    ):
        st.divider()
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    if st.session_state.last_results is None:
        ...
        st.dataframe(
            df.style.format({c: "{:.4f}" for c in df.columns if c != "Epoch"}),
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    if st.button("⚙️ Generate predictions", type="primary"):
        with st.spinner("Running inference on test set…"):
            try:
                response = call_predict(
                    {"model_name": selected_model, "image_size": image_size}
                )
    ...
    if st.session_state.predictions_df is not None:
        ...
        st.caption(
            "Upload this file to the class leaderboard to get your ROC-AUC score."
        )
